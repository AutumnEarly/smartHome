...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	UsartPrintf(USART_DEBUG, "type: %d\r\n",type);
	switch(type)
	{
		UsartPrintf(USART_DEBUG, "type: %d\r\n",type);
		case MQTT_PKT_CMD:															//oneNet命令下发
...
//				}
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				//对req_payload进行JSON解析
				json = cJSON_Parse(req_payload);
				//如果不为空代码解析成功
				if(!json) UsartPrintf(USART_DEBUG,"JSON ERROR:[%s]",cJSON_GetErrorPtr());
				else {
					//获取下发命令
					json_val = cJSON_GetObjectItem(json,"BEEP_SW");
					UsartPrintf(USART_DEBUG,"json_val = %d\r\n",json_val);
					if(json_val->valuedouble > 0) BEEP = 1;
					else BEEP = 0;
				}
				//用完删掉
				cJSON_Delete(json);
