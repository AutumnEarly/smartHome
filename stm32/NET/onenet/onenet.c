...
#define PROID		"qiuchu_smallhome"

#define AUTH_INFO	"qiuchu_smallhome_test"

#define DEVID		"qiuchu_smallhome_0001"
...
//	函数功能：	与onenet创建连接
//
//	入口参数：	timerMax 等待最大时长s
//
//	返回参数：	0-成功	1-失败
//
//	说明：		与onenet平台建立连接
//==========================================================
_Bool OneNet_DevLink(int timerMax)
{
  static int timerStart = 0;
	...
	//超时处理 ESP8266_GetIPD(200); 耗时两秒
	if(status == 1) {
		timerStart++;
		if(timerStart > timerMax / 2) {
			isOnMQTT = 0;
			timerStart = 0;
			UsartPrintf(USART_DEBUG, "ERROR: MQTT TimerOut!\r\n");
			return 0;
		}
	}
	isOnMQTT = 1;
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				//json 不为空则代表获取成功
				if(!json) UsartPrintf(USART_DEBUG,"JSON ERROR [%s]\r\n",cJSON_GetErrorPtr());
				else {
					json_val = cJSON_GetObjectItem(json,"BEEP_SW");
					UsartPrintf(USART_DEBUG,"JSON_val = %d\r\n",json_val->valueint);
					if(json_val->valueint == 1) BEEP = 1,isBeep = 1;
					else if(json_val->valueint == 0) BEEP = 0,isBeep = 0;
				}
		...
		case MQTT_PKT_PUBACK:														//发送Publish消息，平台回复的Ack
			...
			else UsartPrintf(USART_DEBUG, "ERROR:	MQTT Publish\r\n");;
		break;
